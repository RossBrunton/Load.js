...
self.load = (function(self) {
     ...
     *  Before a new file is added to the head, the old one must have been finished downloading, and call
     *  `{@link load.provide}` with it's namespace name.
     * 
     ...
     * 
     * There are a number of different types of packages which can be imported, described by the `TYPE_*` constants.
     *  Note that they exist in the same namespace, so you can't have two different packages with the same name but
     *  different types.
     * 
    ...
    /** The name of the package that is currently being evaluated
     * @type ?string
     * @private
     * @memberof load
     */
     ...
     * @param {string=} type The responseType value, deafult is "json"
    ...
    // ----
    // Multithreading (Probably doesn't work)
    // ----
    ...
    /** Marks that the namespace `name` has been provided, and associates a given function with it
     *
     * The function will be called when the package is evaluated.
     *
     * If there are more packages waiting to be imported and can now be (because they depended on this package) they
     ...
     * @param {function(Window):*} pack The evaluation function which returns the package content.
    ...
    /** Provides a text resource
     *
     * This is similar to `{@link load.provide}`, only it provides a text resource rather than an evalutaion function.
     *
     * @param {string} name The namespace to provide.
     * @param {string} data The string content of this package.
     * @memberof load
     */
     ...
     *  itself).
     *
     * @param {string} name The namespace that has been provided.
     * @memberof load
     */
    load.provideExternal = function(name) {
        ...
        if(name in _packs) {
            ...
            _packs[name] = {file:"about:blank", state:STATE_SEEN, deps:[], size:0, obj:null, type:TYPE_EXT};
        }
     ...
     *
     * This does not evaluate the package.
     * 
    ...
    /** Given a package, if it has not been imported, it is added to `{@link _importSet}` and this function is
     *  called on all its dependancies.
     *
     * This is used to fully papulate the _importSet variable.
     * 
     ...
     *  batch set, then calls `{@link _doImportFile}` to import each of them.
    ...
    var _tryImport = function(trace) {
        ...
        for(var i = 0; i < _importSet.length; i ++) {
            ...
            var okay = true;
            // Loop through all the package's dependencies to see if there are any problmes
            for(var d = 0; d < now.deps.length; d ++) {
                if(now.deps[d].charAt(0) == ">") {
                    //Okay, because its deferred
                    ...
                    // The package depends on something that isn't imported yet. ;_;
                    // Check if they are from the same file, have hope!
    ...
    /** Does the actual action to import the file, either by using AJAX to get it, or adding it as a script tag
     * 
     * @param {string} file The file to add.
     * @param {int} type The type of the file; one of the `TYPE_*` constants.
     ...
     * @param {integer} type The type of the package, one of the `TYPE_*` constants.
     * @memberof load
     */
    ...
    /** Handles a dependecy object in the same format as a `deps.json` file.
     *
     * It reads all the package information out of it, and sets up dependencies as appropriate.
     *
     * If the deps object has dependencies, they will be imported by this.
     * 
     * @param {object} data The package information
     * @param {string} absolutePath The absolute path that all relative paths in this object will be based on.
     * @return {Promise(*)} A promise that will fulfill when the deps object is finished loading.
     * @memberof load
     */
    ...
    /** Download a JSON containing dependencies in the `deps.json` format.
     * 
     * @param {string} path The path to the JSON file.
     * @returns {Promise(object)} A promise that resolves when the file is loaded.
     * @memberof load
     */
    load.loadDeps = function(path) {
        ...
        return _xhrGet(path).then(function(data) {
            return load.loadDepsObject(data, absolutePath).then(function(o) {
                _depFiles[path] = o;
            });
        });
    ...
    /** Marks the given package as also having these dependencies
     *
     * @param {string} pack The package to add dependencies to
     * @param {array<string>} The list of extra dependencies
     * @memberof load
     */
    ...
    /** An error raised if there is dependency problems.
    ...
    /** Invokes the debugger (via the `debugger` operator) to get access to the local closure. 
     * @memberof load
     */
    load.debug = function() {
        debugger;
    ...
    /** Imports all packages, useful for debugging
     * @memberof load
     */
     ...
     * @param {RegExp} The expression to filter by.
     * @memberof load
     */
